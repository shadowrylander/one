...
The One assimilate Emacs packages as Git submodules.  One is a
bare-bones package manager for Emacs packages.

#+TEXINFO: @noindent
This manual is for One version 3.2.0 (v3.2.0-1-g136bb31+1).
...
The One assimilate Emacs packages as Git submodules.
...
One can be used by itself, in which case it has to be bootstrapped.
This is how One originally was intended to be used exclusively but
nowadays you may also choose to use it merely as a secondary package
manager, in which case One itself is installed using Package.

** Use as secondary package manager

To use One as a secondary package manager alongside Package, begin by
installing the former using the latter.  One is only available from
...
package-install RET one RET~.  Doing that should add a verbose variant
of this to your init file:

#+BEGIN_SRC emacs-lisp
  (custom-set-variables
   '(package-selected-packages '(one)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (if (require 'one-elpa nil t)
      (one-elpa-initialize)
    (package-initialize))
#+END_SRC

Just like Package (aka Elpa) defaults to installing packages in ~elpa/~,
One defaults to installing them in ~one/~.  (When using both package
managers, that is.  If used by itself, then One defaults to using
~lib/~.)  If you want to use another directory, then you can do so by
setting the Git variable ~one.drones-directory~.

You should also add a ~Makefile~ containing:

#+BEGIN_SRC makefile
  BORG_SECONDARY_P = true
  include $(shell find -L elpa -maxdepth 1 -regex '.*/one-[.0-9]*' |\
    sort | tail -n 1)/one.mk
#+END_SRC

Despite its title, many things mentioned in the next section are
relevant even when One was installed the way we just did.  Just
saying.
...
One as the package manager.  Most users end up using ~emacs.g~ merely
as a bootstrapping seed and do not merge upstream changes after that.

This collective already assimilates a few drones in addition to ~one~
...
by ~one~ but their use is highly recommended.
...
that imports ~lib/one/one.mk~ and defines an additional target whose
purpose is to make that file and ~lib/one/one.sh~ available.  Run ~make
bootstrap-one~ to clone the ~one~ repository.  That does not completely
setup the ~one~ repository but it makes the latest version of the
...
~one~ submodule) and to build all drones.
...
By default One installs packages inside the ~lib/~ subdirectory, but
since you are starting from scratch, you may choose something else
by setting the Git variable ~one.drones-directory~ locally for this
repository.
...
#+BEGIN_SRC makefile
  ...
  -include $(DRONES_DIR)/one/one.mk

  bootstrap-one:
          @git submodule--helper clone --name one --path $(DRONES_DIR)/one \
          --url git@github.com:emacscollective/one.git
          @cd $(DRONES_DIR)/one; git symbolic-ref HEAD refs/heads/master
          @cd $(DRONES_DIR)/one; git reset --hard HEAD
#+END_SRC

Now you are probably tempted to run ~make bootstrap-one~, but that is
...
#+BEGIN_SRC sh
  git submodule add --name one git@github.com:emacscollective/one.git lib/one
#+END_SRC

Now that ~one~ is available we can build all the assimilated packages (currently
just ~one~ itself) using ~make bootstrap~.

Now it is time to tell Emacs to initialize One instead of Package by
adding a simple ~init.el~ file containing at least:

#+BEGIN_SRC emacs-lisp
  (when (< emacs-major-version 27)
    (setq package-enable-at-startup nil))
  (add-to-list 'load-path (expand-file-name "lib/one" user-emacs-directory))
  (require 'one)
  (one-initialize)
#+END_SRC
...
#+BEGIN_SRC sh
  git commit -m "Assimilate one"
#+END_SRC

Now it is time to assimilate some other essential packages.  You could
do so using ~M-x one-assimilate~, but you would quickly notice that
...
One as described in [[*Assimilation]].  As part of that you should tell
...
One alongside that, then you can proceed as described in [[*Use as
...
One itself to prefer the ~https~ URLS.

#+BEGIN_SRC emacs-lisp
  (setq one-rewrite-urls-alist
...
~one-initialize~.  It should also set ~package-enable-at-startup~ to ~nil~
unless you really want to use both ~one~ and ~package~ at the same time.

- Function: one-initialize

  This function initializes assimilated drones using ~one-activate~.
...
- Command: one-activate clone
  ...
  Unlike ~one-initialize~, this function ignores the Git variable
...
~one-assimilate~, which adds the package's repository as a submodule
and attempts to build the drone.

A safer alternative is to first clone the package without assimilating
it, using ~one-clone~.  This gives you an opportunity to inspect the
...
~one-assimilate~, or remove the clone using ~one-remove~.
...
couldn't be built by using the Emacs command ~one-build~ or ~make
...
The command ~one-insert-update-message~ can be used to generate such
commit messages.

- Key: C-c C-b [in git-commit-mode buffer], one-insert-update-message
...
- Command: one-assimilate package url &optional partially
  ...
  configuring the build steps use ~one-build~ to complete the
  assimilation.

- Command: one-clone package url
...
- Command: one-remove clone

  This command removes the cloned or assimilated package named CLONE,
  by removing the working tree from ~one-drones-directory~, regardless
  of whether that repository belongs to an assimilated package or a
  package that has only been cloned for review using ~one-clone~.  The
  Git directory is not removed.

- Command: one-build clone &optional activate
...
- Function: one-update-autoloads clone &optional path
...
- Function: one-byte-compile clone &optional path
...
- Function: one-makeinfo clone

  This function generates the Info manuals and the Info index for the
  clone named CLONE.

- Function: one-batch-rebuild &optional quick
  ...
  the user init files using ~one-batch-rebuild-init~.
...
- Function: one-batch-rebuild-init
...
One does not provide an update command.  By not doing so, it empowers
...
drone using the command ~one-build~.  This may fail, e.g. due to new
dependencies.
...
By using One you can not only make changes to assimilated packages,
...
have to be in ~~/.emacs.d~ in a shell.  They are implemented in ~one.mk~,
which is part of the ~one~ package.
...
- Command: bootstrap-one

  This target bootstraps ~one~ itself.

- Command: bootstrap

  This target attempts to bootstrap the drones.  To do so it runs
  ~git submodule init~, ~one.sh~ (which see), and ~make build~.

  If an error occurs during the ~one.sh~ phase, then you can just run
  ...
  deinit lib/DRONE~.  When done with ~one.sh~ also manually run ~make
  build~ again.

* Variables

- Variable: one.drones-directory DIRECTORY
...
- Variable: one-drones-directory
  ...
  the ~one~ library and the Git variable ~one.drones-directory~ if set,
  and should not be changed.

- Variable: one-user-emacs-directory
...
The values of these one-specific Git variables have to be set in the
file ~~/.emacs.d/.gitmodules~.  The variables ~one.pushDefault~ and
~submodule.DRONE.remote~ are described in [[*Patching drones]].

- Variable: one.collective REMOTE
  ...
  effect as ~one.pushDefault~.  This special case is only useful for
...
common-sense conventions, One usually does not have to be told how to
build a given drone.  Building is done using ~one-build~, which in turn
usually does its work using ~one-update-autoloads~, ~one-byte-compile~,
and ~one-makeinfo~.
...
variables you can tell One how to build such packages.

- Variable: submodule.DRONE.build-step COMMAND

  By default drones are built using the lisp functions
  ~one-update-autoloads~, ~one-byte-compile~, and ~one-makeinfo~, but
  ...
  #+BEGIN_SRC undefined
    [submodule "mu4e"]
            ...
            build-step = one-update-autoloads
            load-path = mu4e
  #+END_SRC
  ...
  build the package, omitting ~one-update-autoloads~. One silently
  ...
  ~one-makeinfo~ as an additional build-step because the former might
  not generate an Info index file (named ~dir~), which One relies on.

- Variable: one-build-shell-command
...
- Variable: submodule.DRONE.load-path PATH
  ...
  This variable instructs ~one-activate~ to add PATH to the ~load-path~
  instead of the directory it would otherwise have added.  Likewise it
  instructs ~one-byte-compile~ to compile the libraries in that
  ...
  Normally One uses ~elisp/~ as the drone's ~load-path~, if that exists;
...
- Variable: submodule.DRONE.no-byte-compile PATH
  ...
  This variable instructs ~one-byte-compile~ to not compile the library
  ...
  package.  The standard One collective ~emacs.g~ assimilates ~emacsql~,
  ...
  instructs One to not compile ~emacsql-pg.el~.  (Of course if you want
  to use the PostgreSQL back-end and assimilate ~pg~, then you should
  undo that.)

- Variable: submodule.DRONE.recursive-byte-compile BOOLEAN

  Setting this variable to ~true~ instructs ~one-byte-compile~ to compile
  ...
  ~one-byte-compile~ to ignore the containing directory.

- Variable: one-byte-compile-recursive

  Setting this variable to a non-nil value instructs ~one-byte-compile~
  to compile all drones recursively.  Doing so is discouraged.

- Variable: submodule.DRONE.info-path PATH

  This variable instructs ~one-initialize~ to add PATH to
...
- Variable: submodule.DRONE.no-makeinfo PATH
  ...
  This variable instructs ~one-makeinfo~ to not create an Info file for
...
- Variable: one-rewrite-urls-alist
...
build your own tools on top of One.

- Function: one-worktree clone

  This function returns the top-level of the working tree of the
  clone named CLONE.

- Function: one-gitdir clone
...
- Function: one-get clone variable &optional all
...
- Function: one-drones &optional include-variables
  ...
  a member of ~one--multi-value-variables~.  If a property name isn't
  a member of ~one--multi-value-variables~ but it does have multiple
...
- Function: one-clones
  ...
  ~one-drones-directory~ but aren't tracked as submodules.

- Function: one-read-package prompt &optional edit-url
...
- Function: one-read-clone prompt
...
- Function: one--maybe-absorb-gitdir pkg
- Function: one--maybe-reuse-gitdir pkg
- Function: one--restore-worktree pkg
- Function: one--call-git pkg &rest args
- Function: one--expand-load-path drone path
- Function: one--sort-submodule-sections
...
#  LocalWords:  PLIST Texinfo alist args autoload autoloads one
